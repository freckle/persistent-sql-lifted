...
newSavepoint :: (HasCallStack, MonadSqlBackend m) => m Text
newSavepoint = do
...
randomCharacter :: MonadIO m => m Char
...
--  | Release a @SAVEPOINT@
releaseSavepoint :: (HasCallStack, MonadSqlBackend m) => Text -> m ()
releaseSavepoint name = rawExecute ("RELEASE SAVEPOINT " <> name) []

--  | Rollback to a @SAVEPOINT@
rollbackToSavepoint
  :: (HasCallStack, MonadSqlBackend m) => Text -> m ()
